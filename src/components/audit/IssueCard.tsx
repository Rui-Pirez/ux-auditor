...
export function IssueCard({ issue, isQuickWin, expertMode = false, defaultOpen = false }: IssueCardProps) {
  ...
  return (
    <div className={`rounded-xl border transition-all ${open ? 'shadow-sm' : 'hover:shadow-sm'} bg-white border-zinc-100`}>
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1.5 text-sm font-semibold text-zinc-900 leading-snug">{issue.title}</p>
          {!open && (
            <p className="mt-1 text-xs text-zinc-500 line-clamp-2 leading-relaxed">{issue.evidence}</p>
          )}
        </div>
      ...
      {open && (
        <div className="border-t border-zinc-50 px-4 pb-5 pt-4 space-y-4">
          {/* Why it matters */}
          <div className="flex gap-3">
            <div className="mt-0.5 shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-amber-50">
            ...
            <div>
              ...
              <p className="text-xs leading-relaxed text-zinc-600">{issue.whyItMatters}</p>
            </div>
          </div>

          {/* Evidence */}
          <div className="flex gap-3">
            <div className="mt-0.5 shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-blue-50">
              <Lightbulb className="h-3 w-3 text-blue-600" />
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-blue-600 mb-1">Detected on your page</p>
              <p className="text-xs leading-relaxed text-zinc-600 font-mono bg-zinc-50 rounded-lg p-3 border border-zinc-100">
                {issue.evidence}
              </p>
            </div>
          </div>
          ...
          {/* Fix */}
          <div className="flex gap-3">
            <div className="mt-0.5 shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-green-50">
              <Wrench className="h-3 w-3 text-green-600" />
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-green-600 mb-1">Specific fix</p>
              <p className="text-xs leading-relaxed text-zinc-700">{issue.specificFix}</p>
            </div>
          </div>

          {expertMode && (
            <div className="mt-2 rounded-lg bg-violet-50 border border-violet-100 px-3 py-2">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-violet-600">Score impact</p>
              <p className="text-xs text-violet-700 mt-0.5">Fixing this issue recovers <strong>−{issue.scoreDeduction} points</strong> in the {CATEGORY_LABELS[issue.category]} category.</p>
            </div>
          )}
        </div>
      )}
